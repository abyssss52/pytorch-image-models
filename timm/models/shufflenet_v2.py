...
default_cfgs = {
    ...
    'shufflenetv2_100': _cfg(url='https://download.pytorch.org/models/shufflenetv2_x1-5666bf0f80.pth')
...
class BasicUnit(nn.Module):
    def __init__(self, inplanes, outplanes, c_tag=0.5, activation=nn.ReLU, SE=False, residual=False, groups=2):
        ...
        self.SE = SE
        if self.SE:
            self.SELayer = SEModule(self.right_part_out, 2)
...
class ShuffleNetV2(nn.Module):
    ...
    def __init__(self, scale=1.0, in_chans=3, c_tag=0.5, drop_rate=0.0, num_classes=1000, act_layer=nn.ReLU,
                 SE=False, residual=False, groups=2, pad_type='', norm_layer=nn.BatchNorm2d, norm_kwargs=None,
                 global_pool='avg'):
        """
        ...
        self.c = [_make_divisible(chan, groups) for chan in self.num_of_channels[self.scale]]
        self.n = [3, 7, 3]  # TODO: should be [3,7,3]
        self.conv1 = nn.Conv2d(in_chans, self.c[0], kernel_size=3, bias=False, stride=2, padding=1)
        ...
        layer = FinalLayer(self.c[-2], self.c[-1], pad_type, norm_kwargs, norm_layer, act_layer)
        layers = [layer]
        self.final_layers = nn.Sequential(*layers)
        self.global_pool = SelectAdaptivePool2d(pool_type=global_pool)
        # Classifier
        self.classifier = nn.Linear(self.c[-1] * self.global_pool.feat_mult(), self.num_classes)
        self.init_params()
...
def shufflenetv2_100(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = ShuffleNetV2(scale=1.0, in_chans=in_chans, num_classes=num_classes, SE=False, residual=True, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans, filter_fn=_filter_pretrained)
    return model
